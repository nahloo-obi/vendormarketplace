...
def place_order(request):
    ...
    for id in cart_items:
        if id.item.vendor.id not in vendors_ids:

            vendors_ids.append(id.item.vendor.id)
    ...
    #get subtotal for individual restuarant
    for i in cart_items:
        storeitem = Item.objects.get(pk=i.item.id, vendor_id__in=vendors_ids )
        v_id = storeitem.vendor.id

        if v_id in k:
            subtotal = k[v_id]
            subtotal += (storeitem.price * i.quantity)
            k[v_id] = subtotal
        ...
        total_data.update({storeitem.vendor.id:{str(subtotal):str(tax_dict)}})
